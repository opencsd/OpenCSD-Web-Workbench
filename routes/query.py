    ...
    if action.startswith('get-all'):
        ...
        if request.method == 'POST':
            try:
                ...
                # memory -> power로 바꾸기!!
                query = "select cpu_usage_tick, power_usage from instance_node_monitoring \
...
def metric_handler():
    if request.method == 'GET':
        try:
            query = "select cpu_usage_tick, power_usage from instance_node_monitoring order by time desc limit 10 tz('Asia/Seoul')"
